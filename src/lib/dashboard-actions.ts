...
const FALLBACK_DATA = {
  ...
  automation: {
    ...
    conversions: 1847,
    chartData: [
      { name: "Email Sequences", triggers: 4200, responses: 3950 },
      { name: "SMS Campaigns", triggers: 2800, responses: 2650 },
      { name: "Social Media", triggers: 3200, responses: 3010 },
      { name: "Chatbots", triggers: 2100, responses: 1980 },
      { name: "Web Forms", triggers: 2020, responses: 1920 },
      { name: "API Integrations", triggers: 1100, responses: 1040 }
    ]
  },
  sentiment: {
    positive: 68,
  ...
  activity: [
    {
      ...
      title: "New conversion completed",
      description: "Sarah J. completed the premium package purchase",
      user: "Sarah Johnson",
    ...
    },
    {
      ...
      description: "Mike C. scored 88/100 in lead qualification",
      user: "Mike Chen",
...
export async function getDashboardMetrics() {
  ...
  try {
    // Your existing Prisma code here
    const now = new Date()
    const currentMonth = new Date(now.getFullYear(), now.getMonth(), 1)
    const lastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1)
...
export async function getAutomationData() {
  if (!prisma) {
    return FALLBACK_DATA.automation
  }

  try {
    // Your existing implementation with fallback
    const automations = await prisma.automation.findMany({
      where: {
        active: true,
      },
...
export async function getSentimentData() {
  if (!prisma) {
    return FALLBACK_DATA.sentiment
  }
