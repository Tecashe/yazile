// lib/countries.ts - Complete list of countries supported by Pesapal
...
// Helper function to detect user's country from IP
export async function detectUserCountry(): Promise<string> {
  try {
    ...
    if (data.country_code) {
      ...
      const countryExists = COUNTRIES.find(c => c.code === countryCode)
      return countryExists ? countryCode : "US" // Default to US if not found
    }
    return "US"
  } catch (error) {
...
export function formatPhoneWithCountryCode(phone: string, countryCode: string): string {
  ...
  if (!country || !country.phoneCode) return phone
