...
//   let plan = "FREE" as "FREE" | "PRO" | "TEAM"
//   if (priceId === PLAN_PRICE_IDS.PRO) plan = "PRO"
//   if (priceId === PLAN_PRICE_IDS.TEAM) plan = "TEAM"

//   return client.subscription.upsert({
//     where: { userId },
//     update: {
//       plan,
//       status: mapStripeStatusToEnum(status),
//       customerId,
...
// export async function hasAccess(userId: string, requiredPlan: "PRO" | "TEAM") {
...
export async function hasAccess(userId: string, requiredPlan: "PRO" | "TEAM") {
  ...
  // Check if user's plan meets the required level
  if (requiredPlan === "PRO") {
    return ["PRO", "TEAM"].includes(subscription.plan as string)
  }
