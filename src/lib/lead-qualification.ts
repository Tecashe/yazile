...
export async function analyzeMessage(message: string) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        // Progressive delay: 1s, 3s, 6s
        const delay = attempt * 2000 + Math.random() * 1000
        console.log(`Retrying n8n analysis in ${Math.round(delay)}ms...`)
        await new Promise((resolve) => setTimeout(resolve, delay))
      }
    }
  }

  console.warn("Failed to analyze message with n8n after all retries, using fallback")
  return {
    sentiment: 0,
    purchaseIntent: 0,
    questionIntent: false,
...
async function sendLeadToN8n(lead: Lead) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`Successfully sent lead ${lead.id} to n8n`)
      return
    } catch (error) {
  ...
  console.error(`Failed to send lead to n8n after ${maxRetries} attempts`)
}
