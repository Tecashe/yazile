...
} from "../_components/leads/mock-data-generator"

export default function LeadsDashboardPage() {
  const [useMockData, setUseMockData] = useState(true)
  const [isLoading, setIsLoading] = useState(false)
  const [analytics, setAnalytics] = useState(generateMockAnalytics())
  const [recentLeads, setRecentLeads] = useState(generateMockLeads(25))
  const [topLeads, setTopLeads] = useState(generateMockLeads(15))
  const [interactions, setInteractions] = useState(generateMockInteractions(20))
  ...
  const refreshData = () => {
    ...
    setTimeout(() => {
      if (useMockData) {
        ...
        setRecentLeads(generateMockLeads(25))
        setTopLeads(generateMockLeads(15))
        setInteractions(generateMockInteractions(20))
  ...
  const handleMockDataToggle = (enabled: boolean) => {
    ...
    if (enabled) {
      ...
      // Reset to empty state for real data
      setAnalytics({
        totalLeads: 0,
        qualifiedLeads: 0,
        convertedLeads: 0,
        ...
        crmStatus: { connected: false, integrations: [], lastSync: "Just Now"  },
      })
      setRecentLeads([])
      setTopLeads([])
      setInteractions([])
