...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!tenantId || !vfSessionId || !name || !startDate) {
      return NextResponse.json(
        {
          error: "Missing required fields",
          details: "tenantId, sessionId, name, and startDate are required"
        },
        { status: 400 }
      )
    }
    ...
    // Find the exact matching event type by name
    const eventType = eventTypes.find((et: any) => et.name.toLowerCase() === name.toLowerCase())
      ...
      return NextResponse.json(
        {
          ...
          details: `No event type found with name "${name}". Available event types: ${availableNames}`,
    ...
    const availabilityResponse = await fetch(`https://api.calendly.com/event_type_available_times?event_type=${eventType.uri}&start_time=${startDateObj.toISOString()}&end_time=${endDateObj.toISOString()}`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${credentials.accessToken}`,
        "Content-Type": "application/json"
