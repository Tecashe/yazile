...
async function syncToHubSpotT(crmData: any, crmIntegration: any) {
  // Handle both OAuth and direct token storage
  let accessToken = crmIntegration.accessToken || crmIntegration.apiKey;
  let refreshToken = crmIntegration.refreshToken || crmIntegration.apiSecret;
  ...
  if (crmData.leadTier) {
    // Map your custom lead tiers to HubSpot's valid lead status values
    ...
    // Also store the original tier in a custom property
    properties.lead_tier = crmData.leadTier;
  }
...
async function syncToHubSpot(crmData: any, crmIntegration: any) {
  ...
  if (crmData.status) {
    // Only sync status if you have the lead_status property created in HubSpot
    // properties.lead_status = crmData.status;
  }
  ...
  if (crmData.leadTier) {
    ...
    // Only store original tier if you have the lead_tier property created in HubSpot
    // properties.lead_tier = crmData.leadTier;
  }

  if (crmData.score) {
    // Only sync score if you have the lead_score property created in HubSpot
    // properties.lead_score = crmData.score.toString();
  }

  if (crmData.estimatedValue) {
    // Only sync estimated value if you have the estimated_value property created in HubSpot
    // properties.estimated_value = crmData.estimatedValue.toString();
  }
