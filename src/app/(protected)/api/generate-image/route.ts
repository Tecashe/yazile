...
function generateLocalCaption(prompt: string): string {
  // Simple caption generator function that builds a caption locally
  const cleanPrompt = prompt.trim()
  
  // Extract potential hashtag words from the prompt
  const promptWords = cleanPrompt.toLowerCase().split(" ")
  const hashtags = promptWords
  ...
  const captionTemplates = [
    `Captured this amazing ${cleanPrompt} today! What do you think?`,
    `The beauty of ${cleanPrompt} never ceases to amaze me.`,
    `Exploring the wonders of ${cleanPrompt}!`,
    `This ${cleanPrompt} moment was too perfect not to share.`,
    `Finding inspiration in ${cleanPrompt}.`
  ]
  ...
  return `${randomTemplate} ${hashtags} #photography #instagood`
}
...
export async function POST(req: Request) {
  try {
    ...
    const [imageResult, caption] = await Promise.all([
      generateImage(prompt), 
      Promise.resolve(generateLocalCaption(cleanPrompt)) // Use local caption generation
    ])
    ...
    return NextResponse.json({
      ...
      modelUsed: {
        image: imageResult.provider,
        caption: "Local Template Generator", // Updated to reflect local generation
      },
    })
  } catch (error) {
    console.error("Error generating content:", error)
