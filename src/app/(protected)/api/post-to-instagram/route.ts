...
async function instagramApiCall(
  ...
  try {
    logger.info('Making Instagram API call', { url, method: options.method || 'GET' });
    ...
    logger.info('Instagram API response received', { 
      status: response.status, 
    ...
    if (!response.ok) {
      ...
      throw new Error(`Instagram API Error: ${response.status} - ${errorData.error?.message || errorData.message || responseText || 'Unknown error'}`);
    }
    ...
    try {
      ...
      logger.error('Failed to parse Instagram API response as JSON', parseError as Error, { responseText });
    ...
    logger.error('Instagram API call failed', error as Error, { url });
    throw error;
  }
}
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    logger.info('=== Instagram Post Request Started ===');
    ...
    logger.info('Instagram post request validated', {
      userId,
      mediaCount: mediaUrls.length,
      mediaType,
      captionLength: caption.length,
      mediaUrls: mediaUrls.map(url => ({ url, type: getMediaType(url) }))
    });
