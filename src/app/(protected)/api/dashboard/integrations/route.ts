...
import { type NextRequest, NextResponse } from "next/server"
import { onUserInfor } from "@/actions/user"
import { getTenantByUserId, createTenant } from "@/lib/tenant-service"
import { createIntegration, getIntegration } from "@/lib/integration-service"
import type { IntegrationType } from "@prisma/client"

async function getOrCreateTenant(userId: string, userEmail?: string) {
  let tenant = await getTenantByUserId(userId)
  
  if (!tenant) {
    console.log("[getOrCreateTenant] No tenant found, creating new tenant", {
      userId,
      userEmail
    })
    
    // Auto-create tenant with user's email as the name
    const tenantName = userEmail?.split('@')[0] || 'My Organization'
    tenant = await createTenant(userId, tenantName)
    
    console.log("[getOrCreateTenant] Tenant created successfully", {
      tenantId: tenant?.id,
      userId,
      tenantName
    })
...
export async function GET(request: NextRequest) {
  try {
    ...
    const tenant = await getOrCreateTenant(userId, userEmail)
    
    console.log("[GET /api/integrations] Tenant result:", {
      found: !!tenant,
      tenantId: tenant?.id || "FAILED_TO_CREATE",
      userId: userId,
      hasIntegrations: !!tenant?.integrations,
      integrationCount: tenant?.integrations?.length || 0,
      timestamp: new Date().toISOString()
    ...
    if (!tenant) {
      console.error("[GET /api/integrations] Failed to get or create tenant", {
        userId,
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[POST /api/integrations] Tenant result:", {
      found: !!tenant,
      tenantId: tenant?.id || "FAILED_TO_CREATE",
      userId: userId,
      timestamp: new Date().toISOString()
    ...
    if (!tenant) {
      console.error("[POST /api/integrations] Failed to get or create tenant", {
        userId,
        userEmail: userEmail || "N/A",
        timestamp: new Date().toISOString()
      })
