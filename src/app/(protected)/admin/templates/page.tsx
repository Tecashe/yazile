...
//     // Analyze nodes to determine integrations and complexity
...
//     // Determine category based on node types and integrations
...
//     // Determine complexity based on node count and features
...
//     // Estimate setup time based on complexity and integrations
//     let estimatedTime = 15
//     if (complexity === "SIMPLE") estimatedTime = 5
//     else if (complexity === "COMPLEX") estimatedTime = 30
//     estimatedTime += analysis.integrations.length * 2 // +2 min per integration
...
//     // Add integration-specific use cases
...
export default function TemplatesAdminPage() {
  ...
  // Template form component with improved focus management
  const TemplateForm = React.memo(() => {
    ...
    }, [formData.n8nTemplateId, generateSchemaFromTemplate, setFormData])
            ...
            <div className="grid gap-6">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  id="requiredIntegrations"
                  value={localFormData.requiredIntegrations.join(", ")}
                  onChange={(e) => handleLocalCommaSeparatedInput("requiredIntegrations", e.target.value)}
                  className="col-span-3"
                  placeholder="e.g., Slack, Google Sheets, Webhook"
                />
              </div>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  id="expectedOutcomes"
                  value={localFormData.expectedOutcomes.join(", ")}
                  onChange={(e) => handleLocalCommaSeparatedInput("expectedOutcomes", e.target.value)}
                  className="col-span-3"
                  placeholder="e.g., Automated reports, Data sync, Notifications"
                />
              </div>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  id="useCases"
                  value={localFormData.useCases.join(", ")}
                  onChange={(e) => handleLocalCommaSeparatedInput("useCases", e.target.value)}
                  className="col-span-3"
                  placeholder="e.g., Lead management, Customer onboarding, Data backup"
                />
              </div>
  ...
  // Form handlers with proper focus management
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      const { name, value } = e.target
      // Use a ref to prevent re-renders during typing
  ...
  // Fixed comma-separated input handler with focus preservation
  const handleCommaSeparatedInput = useCallback(
    (name: string, value: string) => {
      // Use setTimeout to prevent focus loss during typing
      setTimeout(() => {
        const arrayValue = value
          .split(",")
          .map((item) => item.trim())
