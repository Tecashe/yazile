...
import { Badge } from "@/components/ui/badge"
import { Switch } from "@/components/ui/switch"
import { Plus, Trash2, ArrowLeft, Save, Upload, Loader2, Settings, Eye, Users, Clock, Sparkles } from "lucide-react"
import { toast } from "@/hooks/use-toast"
...
interface BusinessWorkflowTemplate {
  ...
  operations: string[]
  features: string[]
  integrations: any // Json field - can store complex integration objects
  commonUseCase?: string
  isActive: boolean
  isPublic: boolean
  workflowDesign?: any // Json field
...
interface AdminWorkflowDesignerProps {
  initialRequest?: CustomWorkflowRequest | null
  initialTemplate?: BusinessWorkflowTemplate | null
  onBackToDashboard: () => void
  onTemplateSaved: () => void
}
...
const defaultTemplateForm = {
  ...
  operations: [] as string[],
  features: [] as string[],
  integrations: [] as any[], // Now supports complex objects
  commonUseCase: "",
  isActive: true,
  isPublic: false,
  workflowDesign: null,
...
const AdminWorkflowDesigner: React.FC<AdminWorkflowDesignerProps> = ({
  ...
  useEffect(() => {
    if (initialRequest?.aiSuggestions?.workflowDesign) {
      const aiDesign = initialRequest.aiSuggestions.workflowDesign
      setTemplateForm((prev) => ({
        ...prev,
        name: aiDesign.title || initialRequest.title,
        description: aiDesign.description || initialRequest.businessObjective,
        complexity: aiDesign.complexity || "MEDIUM",
        operations: aiDesign.steps?.map((s) => s.title) || [],
        features: aiDesign.benefits || [],
        integrations: aiDesign.integrations || [],
      }))
    } else if (initialTemplate) {
      setTemplateForm({
        name: initialTemplate.name,
        ...
        description: initialTemplate.description,
        complexity: initialTemplate.complexity,
        estimatedSetupTime: initialTemplate.estimatedSetupTime,
        operations: initialTemplate.operations,
        features: initialTemplate.features,
        integrations: Array.isArray(initialTemplate.integrations) ? initialTemplate.integrations : [],
        commonUseCase: initialTemplate.commonUseCase || "",
        isActive: initialTemplate.isActive,
        isPublic: initialTemplate.isPublic,
        workflowDesign: initialTemplate.workflowDesign,
        createdByAdmin: initialTemplate.createdByAdmin,
        originalRequestId: initialTemplate.originalRequestId || "",
        voiceflowProjectId: initialTemplate.voiceflowProjectId || "",
        voiceflowVersionId: initialTemplate.voiceflowVersionId || "",
      })
    }
  ...
  const addArrayItem = useCallback((field: "operations" | "features" | "integrations", value: string) => {
    if (value.trim()) {
      setTemplateForm((prev) => ({
        ...prev,
  ...
  const removeArrayItem = useCallback((field: "operations" | "features" | "integrations", index: number) => {
    setTemplateForm((prev) => ({
      ...prev,
  ...
  const handleSaveTemplate = async (publish: boolean) => {
    ...
    try {
      ...
      // If we want to publish, make sure isPublic is set
      if (publish && templateId) {
        const publishResponse = await fetch(`/api/admin/workflow-templates/${templateId}/publish`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
        ...
        if (!publishResponse.ok) {
          console.warn("Failed to publish template, but template was saved")
        }
      }
  ...
  const ArrayInputSection = ({
    ...
    title: string
    field: "operations" | "features" | "integrations"
    placeholder: string
    description: string
  }) => {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            {templateForm[field].map((item, index) => (
              <div key={index} className="flex items-center justify-between bg-accent/50 p-2 rounded-lg">
                <span className="text-sm">{item}</span>
                <Button variant="ghost" size="sm" onClick={() => removeArrayItem(field, index)}>
                  <Trash2 className="h-4 w-4 text-red-500" />
                </Button>
              </div>
            ))}
            {templateForm[field].length === 0 && (
              <p className="text-muted-foreground text-sm italic">No {field} added yet.</p>
            )}
          </div>
        </CardContent>
      </Card>
  ...
  if (previewMode) {
    return (
      <div className="space-y-6 p-6">
        ...
        {/* Preview Card - How it will appear to users */}
        <Card className="max-w-md mx-auto cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-105 border-2 hover:border-primary/50">
          <CardHeader className="pb-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1">
                <CardTitle className="text-lg">{templateForm.name || "Template Name"}</CardTitle>
                <div className="flex gap-2 mt-1">
                  <Badge
                    variant="outline"
                    className={
                      templateForm.complexity === "SIMPLE"
                        ? "text-green-600 bg-green-50 border-green-200"
                        : templateForm.complexity === "COMPLEX"
                          ? "text-red-600 bg-red-50 border-red-200"
                          : "text-yellow-600 bg-yellow-50 border-yellow-200"
                    }
                  >
                    {templateForm.complexity}
                  </Badge>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <ul className="space-y-1">
                {templateForm.features.slice(0, 3).map((feature, idx) => (
                  <li key={idx} className="flex items-center gap-2 text-sm">
                    <div className="h-3 w-3 rounded-full bg-green-500 flex-shrink-0" />
                    <span className="text-muted-foreground">{feature}</span>
                  </li>
                ))}
                {templateForm.features.length > 3 && (
                  <li className="text-xs text-muted-foreground">+{templateForm.features.length - 3} more features</li>
                )}
                {templateForm.features.length === 0 && (
                  <li className="text-xs text-muted-foreground italic">No features added yet</li>
                )}
              </ul>
            </div>
            ...
            {templateForm.integrations.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {templateForm.integrations.slice(0, 3).map((integration, idx) => (
                    <Badge key={idx} variant="secondary" className="text-xs">
                      {integration}
                    </Badge>
                  ))}
                  {templateForm.integrations.length > 3 && (
                    <Badge variant="secondary" className="text-xs">
                      +{templateForm.integrations.length - 3} more
                    </Badge>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t">
              <div className="flex items-center gap-1 text-sm text-muted-foreground">
                ...
                <span>{templateForm.estimatedSetupTime}min setup</span>
              </div>
              <Button size="sm">
                Configure
    ...
    <div className="space-y-6 p-6">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <Textarea
              ...
              onChange={(e) => handleTemplateFormChange("description", e.target.value)}
              rows={3}
              placeholder="Describe what this workflow does and its benefits..."
            />
          </div>
          <div>
            <Label htmlFor="template-complexity">Complexity</Label>
            <Select
              value={templateForm.complexity}
              onValueChange={(value) => handleTemplateFormChange("complexity", value)}
            >
              <SelectTrigger>
                <SelectValue placeholder="Select complexity" />
              </SelectTrigger>
          ...
          <div>
            <Label htmlFor="estimated-setup-time">Estimated Setup Time (minutes)</Label>
            <Input
              id="estimated-setup-time"
              type="number"
              value={templateForm.estimatedSetupTime}
              onChange={(e) => handleTemplateFormChange("estimatedSetupTime", Number.parseInt(e.target.value) || 15)}
              min="1"
              max="120"
            />
          </div>
          <div className="col-span-2">
            <Label htmlFor="common-use-case">Common Use Case (Optional)</Label>
            <Textarea
              id="common-use-case"
              value={templateForm.commonUseCase}
              onChange={(e) => handleTemplateFormChange("commonUseCase", e.target.value)}
              rows={2}
              placeholder="Describe a typical scenario where this workflow would be used..."
            />
          </div>
          <div>
            <Label htmlFor="voiceflow-project-id">Voiceflow Project ID (Optional)</Label>
      ...
      {initialRequest?.aiSuggestions?.workflowDesign && (
        <Card className="border-l-4 border-green-500">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Sparkles className="h-5 w-5" />
              AI-Suggested Workflow Design (Reference)
            </CardTitle>
            <CardDescription>Use this AI-generated design as a reference to build your template.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <h4 className="font-semibold text-sm mb-2">Suggested Steps:</h4>
                {initialRequest.aiSuggestions.workflowDesign.steps.length === 0 ? (
                  <p className="text-muted-foreground italic text-sm">No steps provided.</p>
                ) : (
                  <ul className="space-y-1">
                    {initialRequest.aiSuggestions.workflowDesign.steps.map((step, index) => (
              ...
              <div>
                <h4 className="font-semibold text-sm mb-2">Suggested Benefits:</h4>
                {initialRequest.aiSuggestions.workflowDesign.benefits.length === 0 ? (
                  <p className="text-muted-foreground italic text-sm">No benefits listed.</p>
                ) : (
                  <ul className="space-y-1">
                    {initialRequest.aiSuggestions.workflowDesign.benefits.map((benefit, index) => (
                      <li key={index} className="text-sm">
                        • {benefit}
                      </li>
                    ))}
            ...
            <div>
              <h4 className="font-semibold text-sm mb-2">Suggested Integrations:</h4>
              <div className="flex flex-wrap gap-2">
                {initialRequest.aiSuggestions.workflowDesign.integrations.map((integration, index) => (
                  <Badge key={index} variant="outline">
                    {integration}
                  </Badge>
                ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <ArrayInputSection
          ...
          field="operations"
          placeholder="e.g., Receive Instagram DM"
          description="Define the step-by-step operations this workflow performs."
        />

        <ArrayInputSection
          title="Key Features"
          field="features"
          placeholder="e.g., Instant lead qualification"
          description="List the main features and benefits users will get."
        />
        ...
        <ArrayInputSection
          ...
          field="integrations"
          placeholder="e.g., Salesforce, Mailchimp"
          description="Specify which external services this workflow needs."
        />
      </div>
    </div>
  )
