...
//     if (processed) {
//       console.log(`Message has already been processed: ${messageKey} (hash: ${messageHash})`)
//       return true
//     }
...
// IMPROVED: More precise message deduplication
export const checkProcessedMessage = async (messageKey: string): Promise<boolean> => {
  try {
    // Create a more specific hash that includes timestamp precision
    const messageHash = crypto.createHash("sha256").update(messageKey).digest("hex")
    ...
    const processed = await client.processedMessage.findFirst({
      where: {
        messageKey: messageHash,
        createdAt: {
          // Reduce time window to 30 seconds to prevent legitimate messages from being blocked
    ...
    if (processed) {
      // Only log if it's actually a recent duplicate (within 10 seconds)
      ...
      if (timeDiff < 10000) {
        ...
        // If it's older than 10 seconds, allow processing (might be a legitimate new message)
...
export const markMessageAsProcessed = async (messageKey: string): Promise<void> => {
  try {
    ...
    // Use upsert with more specific conditions
    await client.processedMessage.upsert({
      where: {
        messageKey: messageHash,
      },
    ...
    // Clean up old processed messages more frequently (older than 5 minutes)
    await client.processedMessage.deleteMany({
      where: {
        createdAt: {
    ...
    console.error("Error marking message as processed:", error)
    // Don't throw error to avoid blocking message processing
  }
}
...
// NEW: Enhanced trigger decision system
...
// NEW: Get automation with full trigger context
