...
const PaymentPopup = ({ isOpen, onClose, onSuccess }: PaymentPopupProps) => {
  ...
  // Separate upgrade and downgrade options
  const upgradePlans = availablePlans.filter(plan => plan.order > currentPlanOrder)
  const downgradePlans = availablePlans.filter(plan => plan.order < currentPlanOrder)

  // Get the first available plan as default (prioritize upgrades)
  const defaultPlan = upgradePlans.length > 0 ? upgradePlans[0] : 
                     downgradePlans.length > 0 ? downgradePlans[0] : null
  ...
  // If no plans are available (shouldn't happen), show a message
              ...
              <div className="p-8">
                ...
                <div className="text-center py-8">
                  ...
                  <Button
                    onClick={handleManageSubscription}
                    variant="outline"
                    className="bg-transparent text-zinc-300 border-zinc-800 hover:bg-zinc-900 hover:text-white rounded-xl transition-colors"
                  >
                    Manage Subscription
                  </Button>
                </div>
        ...
        >
          ...
          >
            <div className="p-8 max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-bold text-white">
                  {isComplete ? "Plan Updated!" : showCardForm ? "Payment" : "Change Your Plan"}
                </h2>
                <Button
                  variant="ghost"
                  size="icon"
                      ...
                      return (
                        <>
                          ...
                          <div>
                            ...
                            <p className="text-xs text-zinc-400">{current.name} - {current.price}{current.period && `/${current.period.replace('per ', '')}`}</p>
                          </div>
                        </>
                      );
                    })()}
              ...
              {isComplete ? (
                ...
                >
                  ...
                  <div className="space-y-2">
                    <h3 className="text-2xl font-semibold text-white">Plan Updated!</h3>
                    <p className="text-zinc-400 max-w-sm">
                      Your account has been updated to <span className="text-white font-medium">{selectedPlan?.name}</span>. 
                      {selectedPlan?.order && selectedPlan.order > currentPlanOrder ? 
                        " All new features are now available." : 
                        " Your plan changes are now active."
                      }
                    </p>
                  </div>
                </motion.div>
                ...
                /* Plan Selection */
                <div className="space-y-6">
                  <Tabs defaultValue={defaultPlan?.id} className="w-full">
                    <TabsList className="w-full bg-zinc-900 rounded-xl p-1 mb-6" style={{display: 'grid', gridTemplateColumns: `repeat(${availablePlans.length}, 1fr)`}}>
                      {availablePlans.map((plan) => (
                        <TabsTrigger
                          key={plan.id}
                          value={plan.id}
                          className="rounded-lg data-[state=active]:bg-white data-[state=active]:text-black text-zinc-400 font-medium transition-all"
                        >
                          {plan.name}
                        </TabsTrigger>
                      ))}
                      ...
                      <TabsContent key={plan.id} value={plan.id} className="space-y-6">
                        ...
                        >
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="text-zinc-400">{plan.description}</p>
                            {/* Show if this is an upgrade or downgrade */}
                            <div className="inline-flex items-center px-2 py-1 bg-zinc-800 rounded-full text-xs">
                              <span className={cn(
                                "text-xs font-medium",
                                plan.order > currentPlanOrder ? "text-green-400" : "text-orange-400"
                              )}>
                                {plan.order > currentPlanOrder ? "↑ Upgrade" : "↓ Downgrade"}
                              </span>
                            </div>
                          </div>
                        </motion.div>
