    ...
    >
      <div className="text-center space-y-3">
        ...
        <div className="space-y-2">
          ...
          {plan.priceKES !== plan.price && <p className="text-xs text-zinc-500">Equivalent to {plan.priceKES}</p>}
        </div>
      </div>
...
const PaymentPopup = ({ isOpen, onClose, onSuccess }: PaymentPopupProps) => {
  ...
  const handlePlanSelect = (plan: (typeof PLANS)[number]) => {
    ...
    if (plan.id === currentPlan) {
      toast({
        title: "You're already on this plan",
        description: `You are currently subscribed to the ${plan.name} plan.`,
        duration: 3000,
      })
      return
    }
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="bg-zinc-950 border-t sm:border border-zinc-800 sm:rounded-2xl shadow-2xl w-full sm:max-w-lg overflow-hidden"
            >
              <div className="p-4 sm:p-8 max-h-[95vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6 sm:mb-8">
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={handleCloseMain}
                    className="rounded-full h-9 w-9 sm:h-10 sm:w-10 text-zinc-400 hover:text-white hover:bg-zinc-900 transition-colors"
                  >
                    <X className="h-5 w-5" />
                  </Button>
                </div>
                      ...
                      <TabsList className="w-full bg-zinc-900 rounded-xl p-1 mb-6 grid grid-cols-3">
                        {PLANS.map((plan) => (
                          <TabsTrigger
                            ...
                            value={plan.id}
                            className={cn(
                              "rounded-lg data-[state=active]:bg-white data-[state=active]:text-black text-zinc-400 font-medium transition-all text-xs sm:text-sm",
                              plan.id === currentPlan && "opacity-50",
                            )}
                            disabled={plan.id === currentPlan}
                          >
                      ...
                      {PLANS.map((plan) => (
                        <TabsContent key={plan.id} value={plan.id} className="space-y-6">
                          <motion.div
                            ...
                            className="text-center space-y-4"
                          >
                            <div className="w-14 h-14 sm:w-16 sm:h-16 mx-auto bg-zinc-900 rounded-2xl flex items-center justify-center">
                            ...
                            <div className="space-y-2">
                              ...
                              <p className="text-sm sm:text-base text-zinc-400">{plan.description}</p>
                            </div>
                          </motion.div>
                          ...
                          <Button
                            ...
                            disabled={plan.id === currentPlan}
                            className={cn(
                              "w-full h-11 sm:h-12 rounded-xl font-medium transition-all text-sm sm:text-base",
                              plan.id === "enterprise"
                                ? "bg-zinc-900 hover:bg-zinc-800 text-white border border-zinc-800"
                                : "bg-white hover:bg-zinc-200 text-black",
                              plan.id === currentPlan &&
                    ...
                    {subscription && subscription.plan !== "FREE" && (
                      <div className="pt-6 border-t border-zinc-800">
                        <Button
                          onClick={handleCancelSubscription}
                          variant="outline"
                          className="w-full h-11 sm:h-12 bg-transparent text-zinc-300 border-zinc-800 hover:bg-zinc-900 hover:text-white rounded-xl transition-colors text-sm sm:text-base"
                        >
                          Cancel Current Subscription
                        </Button>
                      </div>
