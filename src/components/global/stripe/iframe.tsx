...
import { motion, AnimatePresence } from "framer-motion"
import { Button } from "@/components/ui/button"
import { X, Loader2, AlertCircle, Smartphone } from "lucide-react"
...
const PesapalIframeModal = ({ isOpen, paymentUrl, onClose, onSuccess, onCancel }: PesapalIframeModalProps) => {
  const [isLoading, setIsLoading] = useState(true)
  const [showWarning, setShowWarning] = useState(false)
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/90 backdrop-blur-sm p-0 sm:p-4"
          onClick={handleClose}
        >
          <motion.div
            ...
            onClick={(e) => e.stopPropagation()}
            className="relative bg-zinc-950 border-t sm:border border-zinc-800 sm:rounded-2xl shadow-2xl w-full sm:max-w-4xl h-[95vh] sm:h-[90vh] overflow-hidden flex flex-col"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-zinc-800 bg-zinc-900/50 shrink-0">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div className="min-w-0">
                  <h2 className="text-base sm:text-lg font-semibold text-white truncate">Complete Your Payment</h2>
                  <p className="text-xs sm:text-sm text-zinc-400 hidden sm:block">Secure payment powered by Pesapal</p>
                </div>
              </div>
              <Button
                variant="ghost"
                size="icon"
                onClick={handleClose}
                className="rounded-full h-9 w-9 sm:h-10 sm:w-10 text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors shrink-0"
              >
                <X className="h-5 w-5" />
              </Button>
            </div>
              ...
              {isLoading && (
                ...
                >
                  <div className="text-center space-y-4 px-4">
                    ...
                    <div className="space-y-2">
                      <p className="text-white font-medium text-sm sm:text-base">Loading payment page...</p>
            ...
            {/* Footer */}
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-zinc-800 bg-zinc-900/50 shrink-0">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-0">
                <div className="flex items-center gap-2 text-xs text-zinc-500">
                  <svg
                    className="w-3 h-3 sm:w-4 sm:h-4 text-green-500 shrink-0"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                  ...
                  <span className="text-[10px] sm:text-xs">Secured by 256-bit SSL encryption</span>
                </div>
            ...
            <AnimatePresence>
              {showWarning && (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm z-20 p-4"
                  onClick={cancelClose}
                >
                  <motion.div
                    initial={{ scale: 0.9, opacity: 0 }}
                    ...
                    exit={{ scale: 0.9, opacity: 0 }}
                    onClick={(e) => e.stopPropagation()}
                    className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 sm:p-6 max-w-md w-full shadow-2xl"
                  >
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-amber-500/10 flex items-center justify-center shrink-0">
                        <AlertCircle className="w-5 h-5 sm:w-6 sm:h-6 text-amber-500" />
                      </div>
                      <div className="flex-1 space-y-3 min-w-0">
                        <h3 className="text-base sm:text-lg font-semibold text-white">Cancel Payment?</h3>
                        <p className="text-xs sm:text-sm text-zinc-400">
                          Your payment is still in progress. If you close this now, your payment may not be completed
                          and you&apos;ll need to start over.
                        </p>
                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-2">
                          <Button
                            onClick={cancelClose}
                            className="w-full sm:flex-1 h-10 sm:h-auto bg-white text-black hover:bg-zinc-200 rounded-xl text-sm"
                          >
                            Continue Payment
                          </Button>
                          <Button
                            onClick={confirmClose}
                            variant="outline"
                            className="w-full sm:flex-1 h-10 sm:h-auto bg-transparent text-zinc-300 border-zinc-700 hover:bg-zinc-800 rounded-xl text-sm"
                          >
                            Cancel Anyway
                          </Button>
                        </div>
