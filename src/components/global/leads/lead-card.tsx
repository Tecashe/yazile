...
//   const leadCategory = lead.qualificationData?.aiAnalysis?.leadCategory || "Unknown"

//   // Format the timestamp
//   const lastInteractionTime = lead.lastInteractionAt
//     ? formatDistanceToNow(new Date(lead.lastInteractionAt), { addSuffix: true })
//     : "Never"
...
//           <CardTitle className="text-md font-medium">Lead {lead.customerId.slice(0, 8)}...</CardTitle>
...
//                   <div className="text-sm font-medium">{lead.qualificationData.aiAnalysis.sentiment || "Unknown"}</div>

//                   <div className="text-sm">Buying Intent:</div>
//                   <div className="text-sm font-medium">
//                     {lead.qualificationData.aiAnalysis.buyingIntent || "Unknown"}
//                   </div>
//                 </div>
...
//                     {lead.qualificationData.aiAnalysis.summary || "No summary available"}
//                   </p>
//                 </div>
//               </div>
//             </PopoverContent>
...
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover"
import { MessageSquare, Info, ChevronDown, Send } from "lucide-react"
import type { Lead } from "@/types/lead" // Import the Lead type
...
export function LeadCard({ lead, onSendToN8n }: LeadCardProps) {
  ...
  // Format the timestamp - handle both lastInteractionAt and lastContactDate fields
  const lastInteractionTime =
    lead.lastInteractionAt || lead.lastContactDate
      ? formatDistanceToNow(new Date(lead.lastInteractionAt || lead.lastContactDate!), { addSuffix: true })
      : "Never"
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-md font-medium">
            Lead {(lead.customerId || lead.instagramUserId || "").slice(0, 8)}...
          </CardTitle>
          <Badge variant={getBadgeVariant(lead.status)}>{lead.status}</Badge>
        </div>
        <CardDescription className="flex items-center gap-1">
