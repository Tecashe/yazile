...
import { AlertCircle, Brain, Target, Zap, Clock } from "lucide-react"
import { Badge } from "@/components/ui/badge"
    ...
    setTimeout(() => {
      ...
      switch (triggerMode) {
        case "KEYWORDS":
          ...
          triggered = keywords.some((keyword) => message.toLowerCase().includes(keyword.toLowerCase()))
          break

        case "ANY_MESSAGE":
          // Always trigger for any message
          triggered = message.trim().length > 0
          break

        case "SMART_AI":
          // Simulate AI analysis (more sophisticated in production)
          triggered = message.length > 10 && !message.includes("spam") && Math.random() > 0.3
          break

        case "SCHEDULED":
          // Scheduled triggers don't respond to messages
          triggered = false
          simulatedResponse = "This is a scheduled trigger and doesn't respond to messages."
          break
      ...
      if (triggered) {
        simulatedResponse = responseMessage || "Thank you for your message. We'll get back to you soon!"
      }
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Simulate how your automation will respond to messages</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="flex items-center gap-1 bg-background/50">
              {getTriggerModeIcon()}
              <span>
                {triggerMode === "KEYWORDS" && "Keyword Triggers"}
                {triggerMode === "ANY_MESSAGE" && "Universal Response"}
                {triggerMode === "SMART_AI" && "AI Smart Filter"}
                {triggerMode === "SCHEDULED" && "Time-Based Triggers"}
                {triggerMode === "EVENT" && "Event Triggers"}
              </span>
            </Badge>
          </div>

          {triggerMode === "KEYWORDS" && keywords.length > 0 && (
          ...
          <Textarea
            placeholder="Type a message to test your automation..."
            value={message}
            onChange={(e) => setMessage(e.target.value)}
            className="min-h-[100px]"
          />

          <Button onClick={handleSimulate} disabled={loading || message.trim().length === 0} className="w-full">
            {loading ? "Simulating..." : "Test Automation"}
          </Button>
            ...
            <div className="mt-4">
              {isTriggered ? (
                <Alert className="bg-green-500/10 border-green-500/20">
                  ...
                  <AlertTitle>Automation Triggered!</AlertTitle>
                  <AlertDescription>
                    <div className="mt-2 p-3 bg-background rounded-md border">{response}</div>
                  </AlertDescription>
                </Alert>
              ) : (
                <Alert className="bg-amber-500/10 border-amber-500/20">
                  <AlertCircle className="h-4 w-4 text-amber-500" />
                  <AlertTitle>Not Triggered</AlertTitle>
                  <AlertDescription>
                    {response ||
                      (triggerMode === "KEYWORDS"
                        ? "Your message didn't contain any of the trigger keywords."
                        : triggerMode === "SMART_AI"
                          ? "The AI determined this message doesn't require a response."
                          : "Your message didn't meet the trigger criteria.")}
                  </AlertDescription>
                </Alert>
              )}
            </div>
