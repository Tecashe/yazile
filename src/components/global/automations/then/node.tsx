...
const ThenNode = ({ id, theme = { id: "blue", name: "Blue", primary: "light-blue", secondary: "#768BDD" } }: Props) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      // Use the saveListener action to update the response
      const result = await saveListener(
        id, 
        isAiResponse ? "SMARTAI" : "MESSAGE", 
        editedResponse,
        data?.data?.listener?.commentReply||"",
        data?.data?.listener?.commentReplyVariations
      )
      ...
      if (result.status === 200) {
        ...
        toast({
          ...
          description: "Your automation response has been saved successfully.",
        })
        
        // Refetch the automation data to get the latest changes
        queryClient.invalidateQueries({ queryKey: ["automation-info", id] })
      } else {
        throw new Error(result.data || "Failed to save response")
      }
                      ...
                      {!isEditing ? (
                        ...
                        <>
                          ...
                          <Button
                            ...
                            className="h-10 bg-[#3352CC] hover:bg-[#768BDD]"
                  ...
                  {isEditing ? (
                    <div className="space-y-4">
                      <Textarea
                        ...
                        className="min-h-[200px] bg-background-80 border-[#768BDD]/30 focus:border-[#3352CC] text-base resize-none"
                        placeholder="Enter your response message..."
                      />
                      <div className="flex items-center justify-between text-sm text-text-secondary">
                        <span>Character count: {editedResponse.length}</span>
                        {editedResponse.length !== responseText.length && (
                          <span className="text-orange-400">
                            ({editedResponse.length > responseText.length ? '+' : ''}{editedResponse.length - responseText.length} chars)
                          </span>
                        )}
                      </div>
                    </div>
