    ...
    <section className="py-32 px-4 relative overflow-hidden bg-neutral-950">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          {/* Phone mockup */}
          <div
            className={`bg-neutral-900 border-2 ${currentColors.border} rounded-3xl overflow-hidden shadow-2xl ${currentColors.glow}`}
          >
            {/* Instagram DM header */}
            <div className="bg-neutral-800/80 backdrop-blur-sm border-b border-neutral-700 px-6 py-5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-full ${currentColors.bg} flex items-center justify-center ring-2 ${currentColors.border}`}
                >
                ...
                <div>
                  <h3 className="font-semibold text-lg text-white">{scenarios[activeScenario].title}</h3>
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${currentColors.bg} animate-pulse`} />
            ...
            {/* Messages area */}
            <div className="bg-neutral-950/50 backdrop-blur-sm p-6 h-[650px] overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-transparent">
              <AnimatePresence mode="popLayout">
                {displayedMessages.map((message) => (
                  <motion.div
                    key={message.id}
                    ...
                    <div className={`max-w-[85%] ${message.type === "user" ? "order-2" : "order-1"}`}>
                      ...
                      {message.content && (
                        <div
                          className={`px-5 py-3 rounded-2xl ${
                            message.type === "bot"
                              ? "bg-neutral-800/80 backdrop-blur-sm text-white rounded-tl-sm border border-neutral-700"
                              : `${currentColors.bg} ${currentColors.text} rounded-tr-sm border ${currentColors.border}`
                          }`}
                        >
                          <p className="text-sm leading-relaxed whitespace-pre-line">{message.content}</p>
                        ...
                        >
                          {message.buttons.map((button, idx) => (
                            <motion.button
                              ...
                              whileHover={{ scale: 1.05, y: -2 }}
                              whileTap={{ scale: 0.95 }}
                              className="px-4 py-2.5 bg-neutral-800 hover:bg-neutral-700 rounded-full text-sm font-medium border border-neutral-700 transition-all shadow-sm hover:shadow-md text-white"
                            >
                              {button.text}
                            </motion.button>
                          ))}
                        ...
                        >
                          {message.images.map((image, idx) => (
                            <motion.div
                              ...
                              transition={{ delay: 0.4 + idx * 0.15 }}
                              whileHover={{ scale: 1.05, y: -5 }}
                              className="flex-shrink-0 bg-neutral-900 border-2 border-neutral-700 rounded-2xl overflow-hidden cursor-pointer w-48 shadow-lg hover:shadow-xl transition-all"
                            >
                              <div className="relative">
                                <img
                                  src={image.url || "/placeholder.svg"}
                              ...
                              <div className="p-4">
                                <p className="text-sm font-semibold mb-1 text-white">{image.title}</p>
                                {image.price && (
                                  <p className={`text-lg font-bold ${currentColors.text}`}>{image.price}</p>
                                )}
                              </div>
                      ...
                      {/* Timestamp */}
                      <p className="text-xs text-neutral-500 mt-2 px-1">{message.timestamp}</p>
                    </div>
                  </motion.div>
                ))}
                ...
                {isTyping && (
                  <motion.div
                    ...
                    className="flex justify-start mb-4"
                  >
                    <div className="bg-neutral-800/80 backdrop-blur-sm px-5 py-4 rounded-2xl rounded-tl-sm border border-neutral-700">
                      <div className="flex gap-1.5">
                        <motion.div
                          animate={{ scale: [1, 1.3, 1], opacity: [0.5, 1, 0.5] }}
                          transition={{ repeat: Number.POSITIVE_INFINITY, duration: 0.8, delay: 0 }}
            ...
            {/* Input area */}
            <div className="bg-neutral-800/80 backdrop-blur-sm border-t border-neutral-700 px-6 py-4 flex items-center gap-3">
              <button className="p-2 hover:bg-neutral-700 rounded-full transition-colors">
              ...
              <div className="flex-1 bg-neutral-900/50 rounded-full px-5 py-3 border border-neutral-700">
                <input
                  type="text"
                  placeholder="Message..."
                  className="w-full bg-transparent outline-none text-sm placeholder:text-neutral-500 text-white"
                  disabled
                />
              </div>
              <button className="p-2 hover:bg-neutral-700 rounded-full transition-colors">
          ...
          <motion.div
            ...
            className="text-center mt-8 space-y-2"
          >
            <p className="text-lg text-neutral-400">{scenarios[activeScenario].description}</p>
