  ...
  return (
    <section className="py-32 bg-neutral-950 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        />
        <motion.div
          className="absolute bottom-0 right-0 w-full h-full bg-green-500/5"
          animate={{ opacity: [0.5, 0.3, 0.5] }}
          transition={{ duration: 5, repeat: Number.POSITIVE_INFINITY }}
        />
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          <h2 className="text-5xl md:text-7xl font-bold mb-6 text-white">
            The Yazzil
            <span className="block mt-2 text-cyan-400">Transformation</span>
          </h2>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {/* Interactive slider comparison */}
            <div
              className="relative bg-neutral-900 rounded-[3rem] overflow-hidden shadow-2xl mb-12 border-2 border-neutral-800"
              style={{ height: "500px" }}
            >
              {/* Before side (left) */}
              <div className="absolute inset-0 flex items-center justify-center bg-neutral-900">
                ...
                >
                  <div className="w-24 h-24 mx-auto mb-6 rounded-3xl bg-red-500/20 flex items-center justify-center">
                    {BeforeIcon && <BeforeIcon className="w-12 h-12 text-red-400" />}
                  </div>
                  <div className="text-6xl font-bold text-red-400 mb-4">{comparisons[selectedIndex].before.value}</div>
                  <div className="text-2xl text-red-300 font-medium">{comparisons[selectedIndex].before.label}</div>
                  <div className="mt-6 text-neutral-500 text-lg">BEFORE</div>
                </motion.div>
              </div>
              ...
              >
                ...
                >
                  <div className="w-24 h-24 mx-auto mb-6 rounded-3xl bg-green-500/20 flex items-center justify-center">
                    {AfterIcon && <AfterIcon className="w-12 h-12 text-green-400" />}
                  </div>
                  <div className="text-6xl font-bold text-green-400 mb-4">{comparisons[selectedIndex].after.value}</div>
                  <div className="text-2xl text-green-300 font-medium">{comparisons[selectedIndex].after.label}</div>
                  <div className="mt-6 text-neutral-500 text-lg">AFTER</div>
                </motion.div>
              </motion.div>

              {/* Draggable divider */}
              <motion.div
                className="absolute top-0 bottom-0 w-1 bg-cyan-500 shadow-2xl cursor-ew-resize z-10"
                style={{ left: `${dragProgress}%` }}
                drag="x"
                dragConstraints={{ left: 0, right: 0 }}
                dragElastic={0}
              ...
              >
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-cyan-500 rounded-full shadow-xl flex items-center justify-center">
                  <div className="flex gap-1">
              ...
              {/* Labels */}
              <div className="absolute bottom-8 left-8 bg-neutral-800/90 backdrop-blur-sm px-6 py-3 rounded-full font-bold text-red-400 shadow-lg border border-neutral-700">
                WITHOUT YAZZIL
              </div>
              <div className="absolute bottom-8 right-8 bg-neutral-800/90 backdrop-blur-sm px-6 py-3 rounded-full font-bold text-green-400 shadow-lg border border-neutral-700">
                WITH YAZZIL
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <motion.div
                className="bg-neutral-900 rounded-3xl p-8 border-2 border-neutral-800"
                whileHover={{ scale: 1.02, rotate: -1 }}
              >
                <div className="text-sm font-bold text-red-400 mb-2">BEFORE AUTOMATION</div>
                <div className="text-4xl font-bold text-red-400 mb-2">{comparisons[selectedIndex].before.value}</div>
                <div className="text-neutral-400">{comparisons[selectedIndex].before.label}</div>
              </motion.div>
              ...
              <motion.div
                ...
                whileHover={{ scale: 1.02, rotate: 1 }}
              >
                <div className="text-sm font-bold text-green-400 mb-2">AFTER AUTOMATION</div>
                <div className="text-4xl font-bold text-green-400 mb-2">{comparisons[selectedIndex].after.value}</div>
                <div className="text-neutral-400">{comparisons[selectedIndex].after.label}</div>
              </motion.div>
            </div>
          </motion.div>
        </AnimatePresence>
        ...
        >
          <motion.button
            className="px-12 py-6 bg-cyan-500 hover:bg-cyan-600 text-white text-xl font-bold rounded-full shadow-2xl"
            whileHover={{ scale: 1.05, boxShadow: "0 20px 60px rgba(6, 182, 212, 0.4)" }}
            whileTap={{ scale: 0.95 }}
          >
            Start Your Transformation Today
