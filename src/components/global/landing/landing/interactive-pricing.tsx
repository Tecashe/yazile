  ...
  return (
    <section className="py-32 bg-neutral-950 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        />
        <motion.div
          className="absolute bottom-1/4 right-0 w-96 h-96 bg-orange-500/10 rounded-full blur-3xl"
          animate={{ x: [0, -100, 0], scale: [1, 1.3, 1] }}
          transition={{ duration: 25, repeat: Number.POSITIVE_INFINITY }}
        />
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          <h2 className="text-5xl md:text-7xl font-bold mb-6 text-white">
            Simple, Transparent
            <span className="block mt-2 text-cyan-400">Pricing</span>
          </h2>
          ...
          {/* Billing toggle */}
          <motion.div
            className="inline-flex items-center gap-4 bg-neutral-900 rounded-full p-2 shadow-lg border border-neutral-800"
            initial={{ opacity: 0, scale: 0.8 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            transition={{ delay: 0.3 }}
          >
            <button
              onClick={() => setIsAnnual(false)}
              className={`px-6 py-3 rounded-full font-medium transition-all ${
                !isAnnual ? "bg-cyan-500 text-white" : "text-neutral-400"
              }`}
            >
              Monthly
            </button>
          ...
          {plans.map((plan, index) => {
            ...
            return (
              ...
              >
                ...
                <motion.div
                  className={`relative bg-neutral-900 rounded-[2.5rem] p-8 shadow-xl border-2 h-full ${
                    plan.popular ? "border-cyan-500 scale-105" : "border-neutral-800"
                  }`}
                  whileHover={{
                    y: -10,
                    scale: plan.popular ? 1.05 : 1.02,
                ...
                >
                  {/* Glow effect on hover */}
                  <motion.div
                    className="absolute -inset-1 bg-cyan-500/20 rounded-[2.5rem] blur-xl opacity-0"
                    animate={{ opacity: hoveredIndex === index ? 1 : 0 }}
                    transition={{ duration: 0.3 }}
                  />
                  ...
                  <div className="relative">
                    {/* Icon */}
                    <motion.div
                      className="w-16 h-16 rounded-2xl bg-cyan-500 flex items-center justify-center mb-6"
                      whileHover={{ rotate: 360 }}
                      transition={{ duration: 0.6 }}
                    >
                      <Icon className="w-8 h-8 text-white" />
                    ...
                    <div className="mb-8">
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-neutral-400">/{isAnnual ? "month" : "month"}</span>
                      </div>
                      {isAnnual && (
        ...
        <motion.div
          ...
          transition={{ delay: 0.8 }}
        >
          <div className="inline-flex items-center gap-3 bg-neutral-900 rounded-full px-8 py-4 shadow-lg border border-neutral-800">
            <div className="w-12 h-12 rounded-full bg-green-500/20 flex items-center justify-center">
              <Check className="w-6 h-6 text-green-400" />
            </div>
            <div className="text-left">
              <div className="font-bold text-white">30-Day Money-Back Guarantee</div>
              <div className="text-sm text-neutral-400">Not satisfied? Get a full refund, no questions asked.</div>
            </div>
          </div>
        </motion.div>
      </div>
