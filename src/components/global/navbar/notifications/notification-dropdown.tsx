...
import { useState } from "react"
import { Bell, CheckCheck, Clock, ExternalLink, Wifi, WifiOff, RotateCcw } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import {
  DropdownMenu,
...
const notificationColors = {
  LEAD_GENERATED: "bg-green-100 text-green-800 border-green-200",
  LEAD_QUALIFIED: "bg-yellow-100 text-yellow-800 border-yellow-200",
  AUTOMATION_COMPLETED: "bg-blue-100 text-blue-800 border-blue-200",
  AUTOMATION_ERROR: "bg-red-100 text-red-800 border-red-200",
  CAMPAIGN_STARTED: "bg-purple-100 text-purple-800 border-purple-200",
  CAMPAIGN_COMPLETED: "bg-green-100 text-green-800 border-green-200",
  INFLUENCER_APPLIED: "bg-orange-100 text-orange-800 border-orange-200",
  INFLUENCER_ACCEPTED: "bg-green-100 text-green-800 border-green-200",
  WORKFLOW_ACTIVATED: "bg-indigo-100 text-indigo-800 border-indigo-200",
  WORKFLOW_COMPLETED: "bg-green-100 text-green-800 border-green-200",
  CRM_SYNC_SUCCESS: "bg-blue-100 text-blue-800 border-blue-200",
  CRM_SYNC_ERROR: "bg-red-100 text-red-800 border-red-200",
  SYSTEM_ALERT: "bg-gray-100 text-gray-800 border-gray-200",
  BUSINESS_UPDATE: "bg-teal-100 text-teal-800 border-teal-200",
}

export function NotificationDropdown() {
  const { notifications, unreadCount, isLoading, markAsRead, markAllAsRead } = useGeneralNotifications()
  const { isConnected, isReconnecting, connectionAttempts, maxReconnectAttempts, reconnect } =
    useGeneralNotificationStream()
  const [isOpen, setIsOpen] = useState(false)
  const router = useRouter()
  ...
  const getConnectionStatus = () => {
    if (isReconnecting) {
      return {
        ...
        label: `Reconnecting... (${connectionAttempts}/${maxReconnectAttempts})`,
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className="relative">
          ...
          <div className={cn("absolute -bottom-1 -right-1 w-2 h-2 rounded-full", connectionStatus.bgColor)} />
          <span className="sr-only">Notifications</span>
        </Button>
      </DropdownMenuTrigger>
      ...
      <DropdownMenuContent align="end" className="w-96 p-0">
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isConnected && !isReconnecting && (
              <Button variant="ghost" size="sm" onClick={reconnect} className="text-xs h-7">
        ...
        {notifications.length > 0 && (
          <>
            <DropdownMenuSeparator />
            <div className="p-2">
              <Button variant="ghost" className="w-full text-sm bg-transparent" asChild>
                <a href="/dashboard/notifications">
                  View all notifications
                  <ExternalLink className="h-3 w-3 ml-2" />
                </a>
