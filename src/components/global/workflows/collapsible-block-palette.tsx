...
//   const handleDragStart = (e: React.DragEvent, blockType: string) => {
//     console.log("[v0] Starting drag for block type:", blockType)
//     e.dataTransfer.setData("application/json", JSON.stringify({ type: blockType }))
//     e.dataTransfer.effectAllowed = "copy"
...
//                       group relative cursor-grab active:cursor-grabbing
//                       ${sidebarCollapsed ? "w-12 h-12" : "w-full"}
//                       ${draggedBlock === block.type ? "opacity-50" : ""}
//                     `}
//                     onMouseEnter={() => setHoveredBlock(block.type)}
//                     onMouseLeave={() => setHoveredBlock(null)}
//                     draggable
...
export function CollapsibleBlockPalette() {
  ...
  return (
    <TooltipProvider>
      <div
        className={`relative bg-card border-r border-border transition-all duration-300 flex-shrink-0 ${
          sidebarCollapsed ? "w-16" : "w-80"
        }`}
      >
