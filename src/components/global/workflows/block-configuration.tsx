...
import { Plus, Trash2, Settings, CheckCircle, AlertTriangle, Copy } from "lucide-react"
...
export function BlockConfiguration({ node }: BlockConfigurationProps) {
  ...
  const validateConfig = (data: any) => {
    ...
    switch (node.type) {
      case 'trigger':
        if (!data.title?.trim()) errors.push("Trigger title is required")
        break
      case 'text':
        if (!data.message?.trim()) errors.push("Message text cannot be empty")
        break
      case 'condition':
        if (!data.condition) errors.push("Condition type must be selected")
        if (!data.value?.trim()) errors.push("Condition value is required")
        break
      case 'api':
        if (!data.endpoint?.trim()) errors.push("API endpoint URL is required")
        break
      case 'webhook':
        if (!data.url?.trim()) errors.push("Webhook URL is required")
        break
    }
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-2">
          ...
          <Input
            ...
            type="number"
            value={node.data.duration || 5}
            onChange={(e) => handleDataUpdate({ duration: parseInt(e.target.value) || 5 })}
            min="1"
          />
        </div>
        <div className="space-y-2">
          ...
          <Select 
            ...
            onValueChange={(value) => handleDataUpdate({ unit: value })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Configure the properties and behavior of this {node.type} node.
        </p>
      </div>
