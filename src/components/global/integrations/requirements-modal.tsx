...
import { Button } from "@/components/ui/button"
import { Instagram, PhoneIcon as WhatsApp, CheckCircle, AlertCircle, Loader2 } from "lucide-react"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Progress } from "@/components/ui/progress"
import { motion } from "framer-motion"
...
export default function RequirementsModal({ platform, onClose, onConnect }: RequirementsModalProps) {
  const [open, setOpen] = useState(true)
  const [currentStep, setCurrentStep] = useState(1)
  const [loading, setLoading] = useState(false)
  ...
  const handleContinue = async () => {
    ...
    } else if (currentStep === 3 && onConnect) {
      // This is the final step - initiate the OAuth connection
      setIsConnecting(true)
      try {
        await onConnect()
        // Don't close here - let the parent component handle the OAuth flow
        // The modal will be closed when the OAuth callback is received
      } catch (error) {
  ...
  const getFinalStepContent = () => {
    if (isConnecting) {
      return (
        <motion.div
          ...
          className="flex flex-col items-center justify-center py-4"
        >
          <div className="rounded-full bg-blue-100 p-3 dark:bg-blue-900 mb-4">
            <Loader2 className="h-6 w-6 text-blue-600 dark:text-blue-400 animate-spin" />
          </div>
          <h3 className="text-lg font-medium">Connecting...</h3>
          <p className="text-center text-sm text-muted-foreground mt-2 max-w-xs">
            You will be redirected to {platform === "instagram" ? "Instagram" : "WhatsApp"} to authorize the connection.
          </p>
        </motion.div>
      )
    }
    ...
    return (
      ...
      >
        ...
        <p className="text-center text-sm text-muted-foreground mt-2 max-w-xs">
          Click the connect button below to authorize your {platform === "instagram" ? "Instagram" : "WhatsApp"} account.
        </p>
      </motion.div>
    )
  }
  ...
  const getButtonText = () => {
    ...
    return onConnect ? "Connect Account" : "Done"
  }
